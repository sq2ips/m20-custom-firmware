...
const asn1SccUint32 masks2[] = {0x0, 0xFF, 0xFF00, 0xFF0000, 0xFF000000};
...
flag BitStream_AppendByte0(BitStream* pBitStrm, byte v) {
	...
	if (cb) {
		if (pBitStrm->currentByte >= pBitStrm->count) return FALSE;
		mask = (byte)~mask;
		pBitStrm->buf[pBitStrm->currentByte] &= mask;
		pBitStrm->buf[pBitStrm->currentByte] |= (byte)(v << ncb);
	}
...
flag BitStream_AppendByteArray(BitStream* pBitStrm, const byte arr[], const int arr_len) {
	...
	if ((pBitStrm->currentByte + arr_len) * 8 + cb > pBitStrm->count * 8) return FALSE;
...
void BitStream_AppendPartialByte(BitStream* pBitStrm, byte v, byte nbits, flag negate) {
	...
	if (negate) v = masksb[nbits] & ((byte)~v);
...
static void BitStream_EncodeNonNegativeInteger32Neg(BitStream* pBitStrm, asn1SccUint32 v, flag negate) {
	int cc;
...
static flag BitStream_DecodeNonNegativeInteger32Neg(BitStream* pBitStrm, asn1SccUint32* v, int nBits) {
	byte b;
	*v = 0;
	while (nBits >= 8) {
		*v <<= 8;
		if (!BitStream_ReadByte(pBitStrm, &b)) return FALSE;
		*v |= b;
		nBits -= 8;
	}
	if (nBits) {
		*v <<= nBits;
		if (!BitStream_ReadPartialByte(pBitStrm, &b, (byte)nBits)) return FALSE;
		*v |= b;
	}
...
int GetLengthInBytesOfUInt(asn1SccUint64 v) {
	...
	if (v32 < 0x100) return ret + 1;
	if (v32 < 0x10000) return ret + 2;
	if (v32 < 0x1000000) return ret + 3;
	return ret + 4;
}
...
static int GetLengthSIntHelper(asn1SccUint v) {
	...
	if (v32 <= 0x7F) return ret + 1;
	if (v32 <= 0x7FFF) return ret + 2;
	if (v32 <= 0x7FFFFF) return ret + 3;
	return ret + 4;
}

int GetLengthInBytesOfSInt(asn1SccSint v) {
	if (v >= 0) return GetLengthSIntHelper((asn1SccUint)v);
...
flag BitStream_DecodeUnConstraintWholeNumber(BitStream* pBitStrm, asn1SccSint* v) {
	...
	if (!BitStream_DecodeConstraintWholeNumber(pBitStrm, &nBytes, 0, 255)) return FALSE;
...
flag BitStream_DecodeReal(BitStream* pBitStrm, asn1Real* v) {
	...
	if (!BitStream_ReadByte(pBitStrm, &length)) return FALSE;
	if (length == 0) {
		*v = 0.0;
		return TRUE;
	}

	if (!BitStream_ReadByte(pBitStrm, &header)) return FALSE;
...
flag BitStream_EncodeOctetString_no_length(BitStream* pBitStrm, const byte* arr, int nCount) {
	...
	if (cb == 0) {
		...
		while (remainingBytesToSend > 0) {
			int currentBatch =
			    pBitStrm->currentByte + remainingBytesToSend <= pBitStrm->count ? remainingBytesToSend : pBitStrm->count - pBitStrm->currentByte;
...
flag BitStream_DecodeOctetString_no_length(BitStream* pBitStrm, byte* arr, int nCount) {
	...
	if (cb == 0) {
		...
		while (remainingBytesToRead > 0) {
			int currentBatch =
			    pBitStrm->currentByte + remainingBytesToRead <= pBitStrm->count ? remainingBytesToRead : pBitStrm->count - pBitStrm->currentByte;

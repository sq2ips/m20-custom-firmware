...
void adf_write_register_three(void)
{
	uint32_t reg = (3) | ((adf_config.r3.pll_enable & 0x1) << 2) | ((adf_config.r3.pa_enable & 0x1) << 3) |
	               ...
	               ((adf_config.r3.bleed_down & 0x1) << 9) | ((adf_config.r3.vco_disable & 0x1) << 10) | ((adf_config.r3.muxout & 0xF) << 11) |
	               ((adf_config.r3.ld_precision & 0x1) << 15) | ((adf_config.r3.vco_bias & 0xF) << 16) | ((adf_config.r3.pa_bias & 0x7) << 20) |
	               ((adf_config.r3.pll_test_mode & 0x1F) << 23) | ((adf_config.r3.sd_test_mode & 0xF) << 28);
	adf_write_register(reg);
}
...
void adf_4fsk_fone(uint8_t tone)
{
	...
	uint16_t freq_corr_regist_tmp = adf_config.r0.frequency_error_correction;                   // Temporarily store correction value, as
...
void adf_RF_off(void)
{
	...
	adf_set_pa_level(0);
	adf_write_config();
	LL_GPIO_SetOutputPin(RF_Boost_GPIO_Port,

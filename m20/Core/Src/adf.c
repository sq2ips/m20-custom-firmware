...
void adf_turn_off(void) {
	LL_GPIO_ResetOutputPin(RADIO_EN_GPIO_Port, RADIO_EN_Pin);  //TODO - check power consumption. Check if registry is gone.
    LL_GPIO_ResetOutputPin(ADF_CE_GPIO_Port, ADF_CE_Pin);
}
...
void adf_setup(void) {
    ...
    adf_set_frequency(QRG_FSK4);       // Temporarily set freq for CW - it is going to be changed later when turning on TX
...
void adf_RF_off(void) {
    ...
    adf_set_pa_level(0);
    adf_write_config();
    LL_GPIO_SetOutputPin(RF_Boost_GPIO_Port, RF_Boost_Pin); //boost pin high for RF power off. Reduces power by ~15dB
}
